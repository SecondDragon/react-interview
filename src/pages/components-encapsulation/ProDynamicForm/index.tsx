  ...
  return (
    <div>
      <Typography>
        ...
        <Paragraph>
          在我们探讨了“字符串表达式”的利弊之后，我们意识到如果项目<b>没有</b>“后端动态下发 JSON
          且不发版更新”或者“可视化低代码拖拽存储到数据库”的强制要求，那么为了追求极致的开发体验（完美的
          TypeScript 提示、断点调试、极佳性能），使用<b>纯粹的 JavaScript 函数</b>
          才是纯前端配置体系下的真·最佳实践。
        </Paragraph>
        <Alert
          ...
          description={
            <ul style={{ margin: 0, paddingLeft: 20 }}>
              <li>
                <b>彻底抛弃表达式沙箱：</b>移除了危险且低效的 `new Function`
                字符串解析引擎，大大减轻了运行时的负担和安全风险。
              </li>
              <li>
                <b>拥抱原生函数与闭包：</b>`hidden: (values) =&gt; values.age &gt;
                18`。这样的写法不仅可以在 IDE
                中获得无缝的代码高亮和属性补全，更可以直接通过打断点来进行复杂联动逻辑的 Debug。
              </li>
              <li>
                <b>依然保留了注册制与精准依赖更新：</b>通过底层的 `shouldUpdate` 与 `dependencies`
                数组进行阻断，即便是使用了函数调用，也可以完美避免了外层大表单的重绘。
              </li>
            </ul>
          }
          type="success"
      ...
      <Card
        title="互动演示 (Live Demo) - 切换用户类型查看原生 JS 函数联动"
        style={{ marginBottom: '24px' }}
      >

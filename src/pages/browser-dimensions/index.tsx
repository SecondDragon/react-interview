    ...
    <div>
      ...
      <Card title="一、Bug 出现的现象" style={{ marginBottom: '24px' }}>
        <Alert
          ...
          description={
            <div style={{ whiteSpace: 'pre-wrap' }}>{BrowserDimensionsMeta.phenomenon}</div>
          }
          type="warning"
          showIcon
        />
      ...
      <Card title="二、Bug 出现的底层原因" style={{ marginBottom: '24px' }}>
        ...
        <Paragraph style={{ whiteSpace: 'pre-wrap' }}>{BrowserDimensionsMeta.reason}</Paragraph>
        ...
        >
          <Card size="small" title={<Tag color="blue">CSS 像素坐标系</Tag>}>
            ...
            <ul style={{ marginTop: '8px', paddingLeft: '20px', fontSize: '13px' }}>
              <li>
                <Text code>innerWidth</Text>
              </li>
              <li>
                <Text code>clientWidth</Text>
              </li>
              <li>
                <Text code>offsetWidth</Text>
              </li>
              <li>
                <Text code>getBoundingClientRect()</Text>
              </li>
            </ul>
            <Alert
              message="viewport 缩放会改变 CSS 像素的尺寸"
              type="info"
              showIcon
              style={{ marginTop: '8px' }}
            />
          ...
          <Card size="small" title={<Tag color="green">设备独立像素（DIP）</Tag>}>
            ...
            <ul style={{ marginTop: '8px', paddingLeft: '20px', fontSize: '13px' }}>
              <li>
                <Text code>screen.width</Text>
              </li>
              <li>
                <Text code>screen.height</Text>
              </li>
              <li>
                <Text code>screen.availWidth</Text>
              </li>
            </ul>
            <Alert
              message="不受浏览器缩放影响，只与设备硬件有关"
              type="success"
              showIcon
              style={{ marginTop: '8px' }}
            />
          ...
          <Card size="small" title={<Tag color="purple">视觉视口坐标系</Tag>}>
            ...
            <ul style={{ marginTop: '8px', paddingLeft: '20px', fontSize: '13px' }}>
              <li>
                <Text code>visualViewport.width</Text>
              </li>
              <li>
                <Text code>visualViewport.height</Text>
              </li>
              <li>
                <Text code>visualViewport.scale</Text>
              </li>
            </ul>
            <Alert
              message="缩放时动态变化，反映用户实际可见区域"
              type="warning"
              showIcon
              style={{ marginTop: '8px' }}
            />
      ...
      <Card title="三、Bug 如何解决" style={{ marginBottom: '24px' }}>
        <Paragraph>系统理解每个 API 的精确含义，根据场景选择合适的 API。</Paragraph>
      ...
      <Card title="五、Bug 能解决的核心原理" style={{ background: '#f0f5ff' }}>
        <Paragraph style={{ whiteSpace: 'pre-wrap' }}>{BrowserDimensionsMeta.principle}</Paragraph>
        ...
        <ul>
          <li>
            ...
            <Paragraph type="secondary">
              clientWidth = 内容区 + 内边距；offsetWidth = 内容区 + 内边距 + 边框 +
              滚动条；scrollWidth = 所有内容的总宽度（含溢出）。
            </Paragraph>
          </li>
          <li>
            <Text strong>innerWidth 和 clientWidth 有什么区别？</Text>
            <Paragraph type="secondary">
              innerWidth 是窗口级别（含滚动条），clientWidth
              是元素级别（不含滚动条）。document.documentElement.clientWidth 约等于 innerWidth
              减去滚动条宽度。
            </Paragraph>
          </li>
          <li>
            ...
            <Paragraph type="secondary">
              innerWidth 基于 CSS 像素坐标系，缩放会改变 CSS 像素的"尺寸"；screen.width 基于 DIP
              坐标系，只与设备硬件有关，不受浏览器缩放影响。
            </Paragraph>
          </li>
          <li>
            <Text strong>getBoundingClientRect() 有什么性能问题？</Text>
            <Paragraph type="secondary">
              会触发强制重排（forced reflow），频繁调用会影响性能。现代浏览器推荐使用
          ...
          <li>
            ...
            <Paragraph type="secondary">
              优先使用 visualViewport
              API（如果支持），因为它专门反映用户实际看到的区域。兼容性方案可以用 innerHeight
              的变化作为兜底。
            </Paragraph>
          </li>
        </ul>

...
interface AnswerChunk { table: string; content: string; type: 'answer' }
interface FinalChunk { ids: { table: string; id: string }[]; type: 'final' }
...
const AssetCard: React.FC<{
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, paddingBottom: 10, borderBottom: '1px solid #f0f0f0' }}>
        <h3
          style={{
          ...
          onClick={() => { if (isVerified && matched) alert(`跳转: /asset/${matched.id}`); }}
        >
          {table} {isVerified && <span style={{ fontSize: '0.85rem' }}>#</span>}
        </h3>
        <Space>
          {isVerified && (
            <span style={{ background: '#dbeafe', color: '#1e40af', fontSize: '0.72rem', fontWeight: 600, padding: '2px 10px', borderRadius: 12 }}>
              已入库
            </span>
          )}
          {!done && (
            <span style={{ background: '#fef3c7', color: '#92400e', fontSize: '0.7rem', fontWeight: 600, padding: '2px 10px', borderRadius: 12 }}>
              接收中
            </span>
          )}
        </Space>
      </div>
      <div style={{ color: '#6b7280', fontSize: '0.9rem', lineHeight: 1.7 }}>
        {children}
      </div>
    </div>
  );
...
const MemoCard = memo(({ table, content, finalIds, done }: MemoCardProps) => {
  /**
   * 生成单卡片的 Markdown 指令块。
          ...
          <div style={{ maxHeight: 400, overflowY: 'auto' }}>
            ...
            {rawChunks.map((c, i) => (
              <pre key={i} style={{
                ...
                padding: '10px 14px', marginBottom: 10, fontSize: '0.75rem',
                lineHeight: 1.6, color: '#9cdcfe', whiteSpace: 'pre-wrap', wordBreak: 'break-all',
              }}>
  ...
  return (
    <div>
      ...
      <Paragraph>
        ...
        可以通过 <Text code>JSON → Markdown 转换</Text> + <Text code>react-markdown</Text> 自定义渲染的方式，
        实现流式数据到丰富 UI 的优雅映射。
      </Paragraph>
      ...
      <Card title="二、SSE 协议格式" style={{ marginBottom: 24 }}>
        <Paragraph>
          浏览器通过 <Text code>EventSource</Text> API 连接 SSE 端点，
          服务端按 <Text code>data: ...\n\n</Text> 格式逐条推送，浏览器自动处理断线重连。
        </Paragraph>
        <CodeBlock code={SSEDemoExamples.sseSpec} title="SSE 原始数据格式" type="info" language="text" />
      </Card>

      {/* 三、解决方案：JSON → Markdown 转换层 */}
      <Card title="三、解决方案：JSON → Markdown 转换层" style={{ marginBottom: 24 }}>
        <Paragraph>
          核心思路是<Text strong>在 JSON 和 react-markdown 之间加一个转换层</Text>。
          利用 <Text code>remark-directive</Text> 插件的 <Text code>:::xxx</Text> 自定义指令语法，
          将每条 JSON 消息翻译为对应的 Markdown 指令块。
        </Paragraph>
        <CodeBlock code={SSEDemoExamples.buildMarkdown} title="转换层核心代码" type="success" language="typescript" />
        <Divider />
        <CodeBlock code={SSEDemoExamples.reactMarkdownRender} title="react-markdown 拦截 :::asset 指令" type="success" language="tsx" />
      </Card>

      {/* 四、互动演示 */}
      <Card
        title={<span>四、互动演示 <Tag color="blue">Live Demo（逐字符流式聚合）</Tag></span>}
      ...
      <Card title="六、核心原理" style={{ background: '#f0f5ff' }}>
        <ul>
          <li>
            <Text strong>SSE 单向推送：</Text>
            与 WebSocket 不同，SSE 是纯服务端→客户端单向流，基于 HTTP 协议，天然支持断线重连，比轮询省资源。
          </li>
          <li>
          ...
          <li>
            ...
            通过 <Text code>:::asset</Text> 这种自定义指令，将 Markdown 中的结构化块映射到 React 组件，实现"数据→UI"的声明式渲染。
          </li>
          <li>
            <Text strong>流式聚合不闪烁：</Text>
            每个 table 的片段到达时，只更新对应卡片的内容，其他卡片不会重渲染。通过 React.memo 自定义比较函数实现精确控制。
          </li>
          <li>
            <Text strong>O(n) 增量更新：</Text>
            每张卡片独立 ReactMarkdown，只解析自己的内容。新片段不会触发全量 Markdown 重解析，复杂度从 O(n²) 降到 O(n)。
          </li>
        </ul>
      </Card>
    </div>

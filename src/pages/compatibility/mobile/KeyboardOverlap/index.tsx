  ...
  return (
    <Card title="📱 互动演示：模拟 iOS 键盘遮挡" size="small">
      ...
      >
        ...
        >
          <div
            style={{ flex: 1, height: '30px', border: '1px solid #1890ff', borderRadius: '15px' }}
          />
        </div>
  ...
  return (
    <div>
      ...
      <Card title="一、 Bug 出现的现象" style={{ marginBottom: '24px' }}>
        ...
        <ul>
          <li>
            <Text strong>Android:</Text> 窗口高度变小，触发 resize，内容自动挤压上移。
          </li>
          <li>
            <Text strong>iOS:</Text>{' '}
            窗口高度不变，键盘像一个透明层一样覆盖上来，导致位于底部的输入框完全不可见。
          </li>
        </ul>
        <Alert
          message="用户体验痛点"
          description="用户在打字时看不见自己输入了什么，必须手动滑动页面才能露出输入框。"
          type="error"
          showIcon
        />
      </Card>
      ...
      <Card title="二、 Bug 出现的底层原因" style={{ marginBottom: '24px' }}>
        ...
        <Paragraph>{KeyboardOverlapExamples.reason}</Paragraph>
        <Paragraph>
          iOS 的设计理念是尽量不改变 DOM
          布局高度以保证滚动性能。但它引入了“视觉视口”的概念，键盘弹起仅改变可见区域，而不改变 HTML
          根节点的高度，导致 <Text code>fixed</Text> 定位失效。
        </Paragraph>
      </Card>
      ...
      >
        <Paragraph>
          使用 <Text code>VisualViewport API</Text>{' '}
          是唯一的工业级解法。它能实时监听“可见区域”的变化，而不是笨拙地监听窗口 Resize。
        </Paragraph>
        <Divider />
        <KeyboardDemo />
      </Card>
      ...
      <Card title="五、 Bug 能解决的核心原理" style={{ background: '#f0f5ff' }}>
        <ul>
          <li>
            <Text strong>VisualViewport 对象：</Text>该 API 提供了一个独立于标准{' '}
            <Text code>window</Text> 的视口描述对象。它的 <Text code>height</Text>{' '}
            代表了排除键盘、缩放后的真实净空高度。
          </li>
          <li>
            <Text strong>高度差计算补偿：</Text>
            通过 <Text code>window.innerHeight - window.visualViewport.height</Text>
            ，我们可以精准计算出当前键盘在屏幕上占据的物理高度。
          </li>
          <li>
            <Text strong>Offset 动态注入：</Text>
            将计算出的高度差作为 <Text code>padding-bottom</Text> 或 <Text code>bottom</Text>{' '}
            偏移量应用给容器，从而将输入组件顶出键盘区域。
          </li>
        </ul>
      </Card>
    </div>

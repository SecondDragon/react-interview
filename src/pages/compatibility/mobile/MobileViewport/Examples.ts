...
9. JavaScript 宽度 API 详解（面试常考）

   以 iPhone 14 Pro（DIP=390, DPR=3）为例：

   ┌─────────────────────────┬──────────────────────────────────────────┬──────────┬──────────┐
   │ API                     │ 含义                                     │ 未缩放   │ scale=1/3│
   ├─────────────────────────┼──────────────────────────────────────────┼──────────┼──────────┤
   │ window.innerWidth       │ 窗口可视区域 CSS 像素宽度（含滚动条）    │ 390px    │ 1170px   │
   ├─────────────────────────┼──────────────────────────────────────────┼──────────┼──────────┤
   │ document.documentElement│ <html> 元素 CSS 像素宽度（不含滚动条）   │ 390px    │ 1170px   │
   │ .clientWidth            │                                          │          │          │
   ├─────────────────────────┼──────────────────────────────────────────┼──────────┼──────────┤
   │ window.devicePixelRatio │ 物理像素 / CSS 像素比值（DPR）           │ 3        │ 3        │
   ├─────────────────────────┼──────────────────────────────────────────┼──────────┼──────────┤
   │ screen.width            │ 屏幕设备独立像素（DIP）宽度              │ 390px    │ 390px    │
   ├─────────────────────────┼──────────────────────────────────────────┼──────────┼──────────┤
   │ element.offsetWidth     │ 元素 CSS 像素宽度（含边框、内边距）      │ 按实际   │ 按实际×3 │
   ├─────────────────────────┼──────────────────────────────────────────┼──────────┼──────────┤
   │ element.scrollWidth     │ 元素内容 CSS 像素总宽度（含溢出）        │ 按实际   │ 按实际×3 │
   ├─────────────────────────┼──────────────────────────────────────────┼──────────┼──────────┤
   │ element.getBounding     │ 元素 CSS 像素宽度和位置信息              │ 按实际   │ 按实际×3 │
   │ ClientRect().width      │                                          │          │          │
   ├─────────────────────────┼──────────────────────────────────────────┼──────────┼──────────┤
   │ window.visualViewport   │ 视觉视口实时尺寸（用户缩放时动态变化）   │ 390px    │ 390px    │
   │ .width                  │                                          │          │          │
   └─────────────────────────┴──────────────────────────────────────────┴──────────┴──────────┘

   核心规律：
   - ✅ 受 viewport 缩放影响：innerWidth / clientWidth / offsetWidth / scrollWidth / getBoundingClientRect
   - ❌ 不受 viewport 缩放影响：devicePixelRatio / screen.width / visualViewport.width

   为什么 clientWidth 缩放后会变成 1170？
   viewport 缩放改变的是 CSS 像素的"尺寸"。scale=1/3 时，1 个 CSS 像素 = 3 个物理像素，
   原本 390 个 CSS 像素宽度的屏幕，现在可以容纳 1170 个 CSS 像素。
   这就是 viewport 缩放方案中根字体反算容易出错的原因。

   工程建议：
   - 获取设备宽度（DIP）：优先使用 window.innerWidth（未缩放时等于 DIP）
   - 获取 DPR：window.devicePixelRatio
   - 计算物理像素：物理像素 = DIP × DPR = 390 × 3 = 1170
   - viewport 缩放后：dipWidth = clientWidth / dpr（先除以 DPR 还原）`,
};

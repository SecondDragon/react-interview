...
const SafeAreaDemo = () => {
  ...
  return (
    <Card title="📱 互动演示：模拟 iPhone 底部适配" size="small">
      ...
      >
        {/* 模拟页面内容 */}
        <div style={{ padding: '10px', fontSize: '12px' }}>这是页面内容区域...</div>
  ...
  return (
    <div>
      ...
      <Card title="一、 Bug 出现的现象" style={{ marginBottom: '24px' }}>
        <Paragraph>
          在全屏显示的移动应用中（如 iPhone X 及后续机型），由于底部存在 Home
          Indicator（操作条），传统的 <Text code>bottom: 0</Text> 布局会导致按钮与操作条重叠。
        </Paragraph>
        <ul>
          <li>
            <Text strong>交互干扰：</Text>用户点击按钮时容易误触回到主屏幕。
          </li>
          <li>
            <Text strong>视觉缺陷：</Text>内容紧贴边缘，缺乏呼吸感，不符合 iOS 设计规范。
          </li>
        </ul>
      </Card>
      ...
      <Card title="二、 Bug 出现的底层原因" style={{ marginBottom: '24px' }}>
        ...
        <Paragraph>
          Apple 为了实现全面屏，取消了物理 Home
          键，取而代之的是系统级的虚拟操作条。为了不让网页内容被遮挡，W3C 引入了{' '}
          <Text code>viewport-fit=cover</Text> 属性和 <Text code>env()</Text>{' '}
          函数，允许开发者获取这些不可用的屏幕区域大小。
        </Paragraph>
      </Card>
      ...
      >
        <Paragraph>
          使用 <Text code>env()</Text> 的优势在于它是声明式的，由系统自动注入。这意味着即便未来
          Apple 改变了 Home Indicator 的尺寸，你的页面也能自动适应，无需修改代码。
        </Paragraph>
        <Divider />
        <SafeAreaDemo />
      </Card>
      ...
      <Card title="五、 Bug 能解决的核心原理" style={{ background: '#f0f5ff' }}>
        <ul>
          <li>
            ...
            当浏览器设置了 <Text code>viewport-fit=cover</Text>，WebKit 引擎会向 CSS
            运行环境注入四个变量：<Text code>safe-area-inset-top/bottom/left/right</Text>。
          </li>
          <li>
            <Text strong>fallback 机制：</Text>
            <Text code>env(safe-area-inset-bottom, 20px)</Text>{' '}
            支持第二个参数作为默认值，增强了老旧设备的鲁棒性。
          </li>
          <li>
            <Text strong>复合计算：</Text>
            由于 <Text code>env()</Text> 是具体的长度值，它可以与 <Text code>calc()</Text>{' '}
            完美配合，实现复杂的 UI 适配逻辑。
          </li>
        </ul>
      </Card>

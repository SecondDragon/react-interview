  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <Card title="三、 Bug 如何解决" style={{ marginBottom: '24px' }}>
        ...
        <Divider />
        <Alert
          message="为什么防抖（Debounce）无法解决拼音碎片？"
          description={
            <div>
              <Paragraph>
                防抖的本质是<strong>"停止输入后 N 毫秒才执行"</strong>，但它无法区分输入内容是否处于 IME 合成阶段。
              </Paragraph>
              <ul>
                <li>用户输入长拼音（如 <Text code>"zhuang"</Text>）时，中间会有自然停顿思考选字，此时防抖到期触发，携带的仍是未确定的拼音碎片。</li>
                <li>防抖时间太短（如 300ms）→ 长拼音中间停顿导致碎片漏出；防抖时间太长（如 800ms）→ 汉字上屏后需等待过久才响应，体感卡顿。</li>
                <li>核心矛盾：防抖只关心"多久没输入"，不关心输入是否处于<strong>合成状态</strong>，因此永远无法从根本上拦截拼音进入业务逻辑。</li>
              </ul>
            </div>
          }
          type="warning"
        ...
        <Alert
          ...
          description={
            <div>
              ...
              <ul>
                <li><Text strong>360安全浏览器、QQ浏览器、搜狗浏览器、微信内置浏览器、UC浏览器</Text> 等国产双核浏览器，在兼容模式（IE 内核）或旧版 Chromium 内核中，<Text code>isComposing</Text> 属性可能始终返回 <Text code>false</Text>，甚至直接不存在。</li>
                <li>这些浏览器为了兼容老旧网页或降低实现成本，对 <Text code>InputEvent</Text> 的规范实现不完整，导致开发者误以为"加了判断就安全了"，实际上锁完全失效。</li>
                <li>相比之下，<Text code>compositionstart</Text> / <Text code>compositionend</Text> 事件虽然也有时序差异，但触发基本稳定，配合 Ref 标志位是工业级唯一可靠方案。</li>
              </ul>
            </div>
          }
          type="error"
        ...
        <Alert
          ...
          description={
            <div>
              <Paragraph>
                这个方案看起来和工业级实现几乎一样：都有 <Text code>isComposing</Text> 锁，都监听 <Text code>compositionstart</Text> / <Text code>compositionend</Text> / <Text code>input</Text>。唯一的区别是：<strong>它在 <Text code>compositionend</Text> 中只解锁，不主动调用业务逻辑</strong>，而是天真地依赖浏览器在解锁后自动再发一次 <Text code>input</Text>。
              </Paragraph>
              <Paragraph strong>不同内核的时序差异决定了它的死刑：</Paragraph>
              <ul>
                <li>
                  <Text strong>Chromium / WebKit（Chrome、Safari、Edge、国内大部分浏览器）：</Text>
                  时序是 <Text code>input → compositionend</Text>。用户按下空格或数字键选字时，最后一个 <Text code>input</Text> 事件到来时 <Text code>isComposing</Text> 还是 <Text code>true</Text>，被锁拦截；随后 <Text code>compositionend</Text> 触发，锁打开，但浏览器<strong>不会再补发一次 <Text code>input</Text></strong>。导致<strong>最后一个汉字永远无法触发搜索</strong>。
                </li>
                <li>
                  <Text strong>Gecko（Firefox）：</Text>
                  时序是 <Text code>compositionend → input</Text>。<Text code>compositionend</Text> 先解锁，然后 <Text code>input</Text> 正常触发。这个方案在 Firefox 下恰好能工作——但这只是侥幸，不是设计。
                </li>
                <li>
                  <Text strong>信创浏览器（统信 UOS、麒麟系统等国产操作系统自带浏览器）：</Text>
                  这些浏览器通常基于老旧 Chromium（如 80~90 版本）或深度定制的 WebKit，对 <Text code>composition</Text> 事件的实现极不规范：有的 <Text code>compositionend</Text> 根本不触发；有的触发时 <Text code>e.target.value</Text> 尚未更新；有的触发多次；有的触发后<strong>不补发任何 <Text code>input</Text></strong>。依赖"解锁后等 input"在信创环境下是完全不可靠的赌博。
                </li>
              </ul>
              <Paragraph strong>工业级完整实现为什么能兼容所有内核？</Paragraph>
              <ul>
                <li>
                  <strong>不赌浏览器的人品：</strong>在 <Text code>compositionend</Text> 中<strong>主动补偿触发一次</strong> <Text code>doSearch(e.target.value)</Text>，而不是被动等待浏览器发 <Text code>input</Text>。
                </li>
                <li>
                  <strong>Firefox 双触发去重：</strong>Firefox 下会触发两次（<Text code>compositionend</Text> 中一次 + 后续的 <Text code>input</Text> 中一次）。通过 <Text code>AbortController</Text> 和请求 ID 进行物理取消 + 逻辑校验，确保只有最后一次生效。
                </li>
                <li>
                  <strong>Chromium 不丢失：</strong>最后一个 <Text code>input</Text> 被锁拦截，但 <Text code>compositionend</Text> 中补偿触发，完美抹平时序差异。
                </li>
                <li>
                  <strong>信创浏览器兜底：</strong>只要 <Text code>compositionend</Text> 触发（哪怕时序混乱、哪怕只触发一次），业务逻辑一定会被执行，不会因为浏览器漏发 <Text code>input</Text> 而完全失效。
                </li>
              </ul>
            </div>
          }
        ...
        <Paragraph type="secondary">
          以下四个变量任意组合都会导致 IME 事件时序差异：<Text code>Chromium 版本</Text>（Blink 引擎迭代）× <Text code>操作系统 IME 管道</Text>（TSF / InputMethodKit / IBus / Fcitx）× <Text code>输入法框架</Text>（微软拼音 / 搜狗 / Rime / 微信键盘）× <Text code>上屏方式</Text>（空格 / 数字选字）。

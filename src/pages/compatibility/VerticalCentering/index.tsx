  ...
  return (
    <div>
      ...
      <Card title="一、 Bug 出现的现象" style={{ marginBottom: '24px' }}>
        <Paragraph>
          同样的 CSS 代码，按钮里的文字在 iOS 上完美居中，但在某些 Android 机型上却显得整体偏上
          1px，产生视觉上的不和谐。
        </Paragraph>
        <Alert
          message="渲染引擎差异"
          description={
            <List size="small">
              <List.Item>
                <Badge status="success" text="iOS (CoreText):" /> 默认对基线处理非常优雅。
              </List.Item>
              <List.Item>
                <Badge status="warning" text="Android (FreeType):" />{' '}
                渲染时对中文字体上下间距分配不均。
              </List.Item>
            </List>
          }
      ...
      <Card title="二、 Bug 出现的底层原因" style={{ marginBottom: '24px' }}>
        ...
        <Paragraph>{FontCenteringExamples.reason}</Paragraph>
        <Paragraph>
          传统的 <Text code>line-height</Text>{' '}
          是基于字体的排版参数来计算空间的。由于不同系统的渲染引擎对这些“隐藏参数”的解析逻辑不同，导致了即便高度相同，文字在盒子里的物理偏移量也不一致。
        </Paragraph>
      </Card>
      ...
      >
        <Paragraph>
          Flexbox 是目前解决该问题的“银弹”。它通过 CSS
          布局算法强行干预了渲染层，使其不再依赖不稳定的字体基线。
        </Paragraph>
        <Divider />
        <div
          style={{
      ...
      <Card title="五、 Bug 能解决的核心原理" style={{ background: '#f0f5ff' }}>
        <ul>
          <li>
            ...
            Flexbox 居中不关心字体的内部参数。它计算的是 <Text code>Content Box</Text>{' '}
            的剩余空间，并将其平分到上下两侧，这是一种纯数学的几何分配。
          </li>
          <li>
            <Text strong>解除字体绑定：</Text>
            通过取消 <Text code>line-height</Text>{' '}
            对行高的撑开作用，文字的渲染高度变回了由字号决定的自然高度，从而规避了 Leading
            值带来的不均。
          </li>
          <li>
            <Text strong>亚像素对齐补吸附：</Text>
            在开启硬件加速的情况下，Flex 引擎会配合 GPU 进行亚像素对齐，确保即使在非 Retina
            屏幕上，文字边缘也能尽可能贴合中轴线。
          </li>
        </ul>
      </Card>

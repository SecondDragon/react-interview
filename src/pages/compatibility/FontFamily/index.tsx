  ...
  return (
    <div>
      ...
      <Card title="一、 Bug 出现的现象" style={{ marginBottom: '24px' }}>
        <Paragraph>同样的网页，在不同操作系统下视觉体验差异巨大。</Paragraph>
        <ul>
          <li>
            <Text strong>Windows 锯齿感：</Text>由于 ClearType 渲染机制，部分字体在 Windows
            下显得生硬。
          </li>
          <li>
            <Text strong>信创系统回退：</Text>国产 Linux
            系统由于缺少微软雅黑，常回退到极难看的宋体，导致排版错位。
          </li>
          <li>
            <Text strong>Emoji 乱码：</Text>缺乏跨平台字体栈会导致表情包显示为方块。
          </li>
        </ul>
      </Card>
      ...
      <Card title="二、 Bug 出现的底层原因" style={{ marginBottom: '24px' }}>
        ...
        <Paragraph>
          每个操作系统都有自己的“亲儿子”字体。如果 CSS
          只写死一种字体，在不具备该字体的平台上，浏览器会随机选择一种系统字体，彻底破坏 UI
          的一致性。
        </Paragraph>
      ...
      <Card title="三、 Bug 如何解决" style={{ marginBottom: '24px' }}>
        <Paragraph>建立“阶梯式”回退机制，确保每一类设备都能用到其最优字体。</Paragraph>
        <CodeDiff
      ...
      <Card title="五、 Bug 能解决的核心原理" style={{ background: '#f0f5ff' }}>
        <ul>
          <li>
            ...
            这是核心！浏览器在渲染每个字符时，会按顺序遍历字体栈。将英文字体（如
            Arial）排在前面，中文（如微软雅黑）排在后面，可以实现“英文用
            Arial，中文自动回退到微软雅黑”的完美效果。
          </li>
          <li>
            <Text strong>系统变量 (Magic Keywords)：</Text>像 <Text code>-apple-system</Text>{' '}
            是浏览器的“暗号”，它告诉引擎直接调用操作系统当前默认的、经过高度优化的 UI 字体。
          </li>
          <li>

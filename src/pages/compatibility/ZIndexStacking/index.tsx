  ...
  return (
    <div>
      ...
      <Card title="一、 Bug 出现的现象" style={{ marginBottom: '24px' }}>
        <Paragraph>
          开发者给一个弹窗或 Loading 遮罩层设置了 <Text code>z-index: 9999</Text>
          ，但在页面上它仍然被一个 <Text code>z-index: 10</Text> 的普通侧边栏挡住了。
        </Paragraph>
        <Alert
          message="无效的权重"
          description="在某些特殊的 CSS 属性干扰下，全局的 z-index 比较机制会发生失效。"
          type="error"
          showIcon
        />
      </Card>
      ...
      <Card title="二、 Bug 出现的底层原因" style={{ marginBottom: '24px' }}>
        ...
        <Paragraph>{ZIndexExamples.reason}</Paragraph>
        <Paragraph>
          当父元素设置了 <Text code>transform</Text>、<Text code>opacity</Text>、
          <Text code>filter</Text>{' '}
          等属性时，它会强行开辟一个独立的渲染层。在这个层内部，所有的子元素都在比高低，但对于层外部来说，这一层所有的东西都被整体视为一个单一的原子层级。
        </Paragraph>
      </Card>
      ...
      >
        <Paragraph>
          <Text code>Portal</Text>{' '}
          的本质是“物理搬家”。它让组件在逻辑上仍然属于当前的组件树，但在物理 DOM 结构上却被挂载到了{' '}
          <Text code>body</Text> 下。这样它就彻底逃离了父级复杂的 CSS 环境限制。
        </Paragraph>
        <Divider />
        <ZIndexDemo />
      </Card>
      ...
      <Card title="五、 Bug 能解决的核心原理" style={{ background: '#f0f5ff' }}>
        <ul>
          <li>
            ...
            浏览器在生成渲染位图时，并不是从头到尾画一遍。它会根据 Stacking Context
            将页面拆成多个层。每一层都是一个封闭的坐标系。
          </li>
          <li>
          ...
          <li>
            ...
            通过将 DOM
            节点挂载到根部（Body），该组件便直接成为了根级层叠上下文的直接子元素。在这一层，它的{' '}
            <Text code>z-index: 9999</Text> 才是真正面向全站有效的全局最高权重。
          </li>
        </ul>
      </Card>
    </div>

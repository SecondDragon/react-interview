...
const TabSyncDemo = () => {
  ...
  return (
    <Card title="🔄 互动演示：模拟跨窗口通信" size="small">
      ...
      <Button onClick={sendLogout} type="primary" danger>全站安全退出</Button>
      <Divider />
      <List 
        size="small" 
        header={<div>通信日志:</div>}
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1000px', margin: '0 auto' }}>
      ...
      <Card title="一、 Bug 出现的现象" style={{ marginBottom: '24px' }}>
        ...
        <Alert message="安全合规漏洞" description={TabSyncExamples.phenomenon} type="error" showIcon />
      </Card>
      ...
      <Card title="二、 Bug 出现的底层原因" style={{ marginBottom: '24px' }}>
        ...
        <Paragraph>
          浏览器默认将每个标签页作为独立的进程或上下文运行。在一个页面中清除 Session 或 Cookie，其他页面如果由于 JS 变量缓存或未刷新页面，无法实时感知到后端凭证的失效。
        </Paragraph>
      </Card>
      ...
      >
        <Paragraph>
          <Text code>BroadcastChannel</Text> 是最优雅的跨标签通信手段。它不依赖于存储介质的变更，消息通过浏览器内核直接在内存间传递，实时性极高。
        </Paragraph>
        <Divider />
        <TabSyncDemo />
      </Card>
      ...
      <Card title="五、 Bug 能解决的核心原理" style={{ background: '#f0f5ff' }}>
        <ul>
          ...
          <li>
            ...
            与监听 <Text code>localStorage</Text> 的变动事件相比，BroadcastChannel 不需要进行数据序列化和磁盘写入，响应时间缩短了几个数量级。
          </li>
          <li>

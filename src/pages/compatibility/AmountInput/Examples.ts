...
export const AmountInputExamples = {
  ...
  reason:
    '1. 程序化赋值冲突：当你通过代码 `input.value = newValue` 强行修改输入框的值时，浏览器无法判断这是‘内容修正’还是‘完全替换’。为了安全，现代浏览器会自动将光标重置到字符串的最末尾。\n2. 索引偏移：千分位格式化会动态插入‘逗号’。即便你记住了旧的光标位置（如索引 3），但在新字符串中，由于多了一个逗号，索引 3 对应的数字已经发生了变化，导致光标虽然没跳到末尾，但位置也对不上。',
  phenomenon:
    '用户想在‘1,00|0’中间插入一个‘5’，结果输入后光标瞬间跳到了‘15,000|’的最右侧。用户不得不再次点击中间位置才能继续输入，这在需要输入长串金额的银行场景中是交互灾难。',
  bad: `

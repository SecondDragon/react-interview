    ...
    <div style={{ padding: '24px', height: '100%', display: 'flex', flexDirection: 'column' }}>
      <Typography style={{ marginBottom: 24 }}>
        ...
        <Alert 
          ...
          description={
            <ul>
              <li><Text strong>稳定性：</Text> 观察快速滚动时是否有白屏（Virtuoso 优化了首屏渲染与缓冲区策略）。</li>
              <li><Text strong>纠偏：</Text> 图片异步加载出来后，观察滚动条是否会剧烈抖动（Virtuoso 内部会自动补偿高度偏移）。</li>
              <li><Text strong>代码复杂度：</Text> 无需手写 ResizeObserver 和位置计算映射表。</li>
            </ul>
          }
        />
      </Typography>
          ...
          components={{
            Footer: () => (
              <div style={{ padding: '20px', textAlign: 'center' }}>
                {loading ? <Spin tip="正在加载更多顶级优化数据..." /> : (hasMore ? null : <Text type="secondary">--- 已加载全部 300 条动态 ---</Text>)}
              </div>
            )
          }}
        />

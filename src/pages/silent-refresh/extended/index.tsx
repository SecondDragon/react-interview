...
const SilentRefreshExtended: React.FC = () => {
  return (
    <div>
      ...
      <Paragraph>
        Promise 链替换模式不仅用于 Token 刷新，还可以推广到多种网络异常场景。 核心模式始终是：
        <Text code>拦截器捕获失败 → 执行修复逻辑 → return 新的 Promise</Text>。
      </Paragraph>
      ...
      <Card
        title={
          <span>
            互动演示 <Tag color="blue">Live Simulation</Tag>
          </span>
        }
      ...
      <Card title="模式 1：指数退避自动重试" style={{ marginBottom: 24 }}>
        <Paragraph>
          场景：网络抖动、服务临时不可用。通过<Text strong>指数退避</Text>（1s → 2s →
          4s）避免重试风暴。
        </Paragraph>
        <CodeBlock
          code={ExtendedExamples.retry}
          title="5xx 自动重试（最多 3 次）"
          type="info"
          language="typescript"
        />
      </Card>
      ...
      <Card title="模式 2：接口降级 / 故障转移" style={{ marginBottom: 24 }}>
        ...
        <CodeBlock
          code={ExtendedExamples.fallback}
          title="主接口超时 → 自动切备用"
          type="info"
          language="typescript"
        />
      ...
      <Card title="模式 4：限流等待（429）" style={{ marginBottom: 24 }}>
        ...
        <CodeBlock
          code={ExtendedExamples.rateLimit}
          title="429 限流 → 等待 Retry-After 后重试"
          type="info"
          language="typescript"
        />
      </Card>
      ...
      <Card title="🚀 统一拦截器：组合所有模式" style={{ marginBottom: 24 }}>
        ...
        <CodeBlock
          code={ExtendedExamples.unifiedInterceptor}
          title="统一拦截器：按优先级兜底"
          type="success"
          language="typescript"
        />
      ...
      <Card title="核心原理" style={{ background: '#f0f5ff' }}>
        <Paragraph>无论哪种扩展模式，本质都是同一个公式：</Paragraph>
        <pre
          style={{
            background: '#1e1e1e',
            color: '#d4d4d4',

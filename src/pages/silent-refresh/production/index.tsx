...
const ConcurrencyDemo: React.FC = () => {
  ...
  const addLog = useCallback(
    (msg: string, type: 'info' | 'success' | 'error' | 'warning' = 'info', endpoint?: string) => {
  ...
  return (
    <div>
      ...
      <Paragraph>
        同时发起 <Text strong>4 个请求</Text>，Token
        均已过期。观察刷新锁（isRefreshing）如何保证只刷新 <Text strong>1 次</Text>
        ，其余请求等待共享结果。
      </Paragraph>
            ...
            dataSource={[
              {
                key: '1',
                scenario: '无去重',
                requests: '1(auth) × 4 + 4(retry) = 8',
                refreshCalls: '4 次',
              },
              {
                key: '2',
                scenario: '有去重（本演示）',
                requests: '1(auth) + 4(retry) = 5',
                refreshCalls: '1 次',
              },
            ]}
            columns={[
...
const SilentRefreshProduction: React.FC = () => {
  ...
  return (
    <div>
      ...
      <Paragraph>
        基础方案解决了"上层无感知"问题，但在<Text strong>并发场景</Text>
        下仍然存在缺陷。生产级方案需要三个关键设计。
      </Paragraph>

      {/* 一、并发去重 */}
      <Card title="一、并发去重：多个请求同时 401 只刷新一次" style={{ marginBottom: 24 }}>
        <Paragraph>
          场景：页面初始化时同时请求用户列表、订单列表、统计数据。如果 Token 已过期，3
          个请求几乎同时收到 401。
        </Paragraph>
      ...
      <Card title="三、主动预判：Token 快过期时提前刷新" style={{ marginBottom: 24 }}>
        <Paragraph>
          与其等 401 了再被动修复，不如<Text strong>在 Token 过期前主动刷新</Text>
          。这需要在请求拦截器中解析 JWT 的过期时间。
        </Paragraph>
        <CodeBlock
          code={ProductionExamples.preemptiveCode}
          title="请求拦截器：提前 5 分钟刷新"
          type="success"
          language="typescript"
        />
        <Alert
      ...
      <Card title="四、响应头自动续签（零延迟方案）" style={{ marginBottom: 24 }}>
        ...
        <CodeBlock
          code={ProductionExamples.autoRenewCode}
          title="响应拦截器：自动读取续签头"
          type="success"
          language="typescript"
        />
      ...
      <Card title="六、核心原理总结" style={{ background: '#f0f5ff' }}>
        <ul>
          <li>
            <Text strong>Promise 链替换</Text>：错误拦截器 return 新 Promise，上层无感知
          </li>
          <li>
            <Text strong>并发去重</Text>：isRefreshing 锁 + subscribers 观察者队列，保证只刷新一次
          </li>
          <li>
            <Text strong>主动预判</Text>：解析 JWT exp 字段，提前 5 分钟刷新，在 401 前就完成
          </li>
          <li>
            <Text strong>响应头续签</Text>：后端在 x-access-token 头中下发新 Token，零额外请求
          </li>
        </ul>
      </Card>
    </div>

...
export const principleData = {
  ...
  points: [
    ...
    {
      title: '任务如何被切分：workInProgress + shouldYield',
      content:
        'React 的 render 阶段会创建一棵 workInProgress 树并逐节点遍历（beginWork/completeWork）。每处理完一个节点，React 会调用 scheduler 包的 shouldYield() 检查当前帧剩余时间。如果剩余时间不足 5ms 或有更高优先级任务，React 会保存当前遍历到的节点（workInProgress）并退出渲染；等浏览器空闲后再通过 scheduleCallback 恢复遍历。因此“切分”不是把一棵大树预先切成几块，而是每处理一个节点就检查一次是否该让出主线程。',
    },
    {
      title: 'Lane 模型',
      content:
